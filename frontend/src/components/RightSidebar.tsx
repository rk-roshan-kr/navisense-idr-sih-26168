...
export const RightSidebar: React.FC<RightSidebarProps> = ({ telemetry }) => {
  ...
  useEffect(() => {
    ...
    const idrErr = driftM;
    // Theoretical EKF & Raw INS divergence during blackout
    let ekfErr = idrErr * 1.5;
    let rawErr = idrErr * 3.2;
    if (isBlackout) {
      const elapsed = telemetry.blackout_elapsed_s;
      rawErr = Math.max(idrErr, 0.5 * 0.8 * elapsed * elapsed); // quadratic divergence E ~ 0.5*a*t^2
  ...
  return (
    <div className="legacy-right-panel">
      ...
      {/* 5. Live Error Chart */}
      <div className="legacy-section" style={{ flex: '1 1 auto', minHeight: '140px', display: 'flex', flexDirection: 'column' }}>
        <div className="legacy-section-title">Error Over Time (m)</div>
        <div style={{ flex: 1, position: 'relative', width: '100%', minHeight: '90px' }}>
          <canvas ref={canvasRef} width={250} height={90} style={{ width: '100%', height: '100%', display: 'block' }} />
        </div>
      ...
      {/* 7. Live Event Log */}
      <div className="legacy-section" style={{ flex: '0 0 auto', maxHeight: '130px', overflowY: 'auto' }}>
